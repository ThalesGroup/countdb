...
class AthenaMockQueryHandler(ABC):
    ...
    def run_query(
        self, query: str, exec_params: Optional[List], context: MyAthenaBackendContext
    ):
        m = (
            self.get_pattern().fullmatch(query)
            if self.full_match()
            else self.get_pattern().match(query)
        )
        if m is not None:
...
class MyAthenaBackend(AthenaBackend):
    ...
    def get_query_results(self, exec_id: str) -> QueryResults:
        ...
        for handler in self._query_result_handlers:
            result = handler.run_query(
                query_exec.query, query_exec.execution_parameters, self._context
            )
            if result:
                return result
