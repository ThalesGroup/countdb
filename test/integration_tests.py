...
class TestSimpleIntegrationDataset:
    ...
    def test_collect_simple_counter_collect(self):
        result = handle_event({"operation": "collect", "dataset": "simple_dataset", "override": True})
        assert {"exists": 0, "operation": "collect", "success": 1}.items() <= result.items()
        result = handle_event({"operation": "collect", "dataset": "simple_dataset", "override": False})
        assert {"exists": 1, "operation": "collect", "success": 0}.items() <= result.items()

    def test_collect_and_aggregate_full_month(self):
        start_day = "2024-05-01"
        end_day = "2024-05-31"
        result = handle_event({"operation": "collect", "dataset": "simple_dataset", "override": False,
                               "from_day": start_day, "to_day": end_day})
        assert result["exists"] + result["success"] == 31
        week_start_day = "2024-05-06"
        week_end_day = "2024-05-12"
        result = handle_event({"operation": "aggregate", "dataset": "simple_dataset", "interval": "week",
                               "from_day": week_start_day, "to_day": week_end_day, "override": True})
        assert {"exists": 0, "operation": "aggregate", "success": 1}.items() <= result.items()

        weekly_counters = list(get_query_results_dict(
        ...
        assert weekly_counters == [{'counter_id': '1', 'str_key1': 'simple1', 'int_key1': None, 'value': '7'},
        ...
        result = handle_event({"operation": "aggregate", "dataset": "simple_dataset", "interval": "week",
                               "from_day": start_day, "to_day": end_day, "override": False})
        assert result["exists"] >= 1
        assert result["exists"] + result["success"] == 3

        result = handle_event({"operation": "aggregate", "dataset": "simple_dataset", "interval": "month",
                               "from_day": start_day, "to_day": end_day, "override": True})
        assert {"exists": 0, "operation": "aggregate", "success": 1}.items() <= result.items()
        monthly_counters = list(get_query_results_dict(
            """SELECT str_key1, value 
               ...
               WHERE dataset = 'simple_dataset' 
                     ...
                     AND counter_id = 1
               ORDER BY str_key1"""))
        assert monthly_counters == [{'str_key1': 'simple1', 'value': '31'},
    ...
    def _run_detect(method: str) -> dict:
        ...
        handle_event({"operation": "detect", "dataset": "simple_dataset", "interval": "day",
                      "from_day": start_day, "to_day": end_day, "override": True,
                      "method": method})
        highlights = list(get_query_results_dict(
            f"""SELECT *
        ...
        assert highlights[0].items() >= {'method': method,
                                         'counter_id': '2',
                                         'intervals': f'[{", ".join(days_range(start_day, end_day))}]',
                                         'dataset': 'simple_dataset',
                                         'interval_type': 'day'}.items()
        return highlights[0]

    def test_detect_peak(self):
        result = self._run_detect(method="Peak")
        assert result.items() >= {'int_key': '[1]', 'sub_method': "EmpiricalRule",
                                  'anomalies': '[2024-05-15]',
                                  'vals': str([100 if i == 15 else 1 for i in range(1, 32)])}.items()

    def test_detect_trend(self):
        result = self._run_detect(method="Trend")
        assert result.items() >= {'int_key': '[2]', 'sub_method': None,
                                  'anomalies': None,
                                  'vals': str(list(range(1, 32)))}.items()

    def test_detect_pattern(self):
        result = self._run_detect(method="Pattern")
        assert result.items() >= {'sub_method': 'RecentIncrease',
                                  'int_key': '[2]',
                                  'anomalies': '[2024-05-31]',
                                  'vals': str(list(range(1, 32)))}.items()

    def test_detect_max(self):
        handle_event({"operation": "clear", "dataset": "simple_dataset", "table": "daily_max_counters"})
        handle_event({"operation": "detect", "dataset": "simple_dataset", "interval": "day",
                      "from_day": "2024-05-01", "to_day": "2024-05-10", "override": True,
                      "method": "max"})
        max_records = list(get_query_results_dict(
        ...
        assert max_records[0] == {'counter_id': '1', 'int_key1': None, 'str_key1': 'simple1', 'value': '1',
                                  'max_interval': '2024-05-01', 'prev_value': None}
        assert max_records[1] == {'counter_id': '1', 'int_key1': None, 'str_key1': 'simple2', 'value': '2',
                                  'max_interval': '2024-05-01', 'prev_value': None}
        assert max_records[2] == {'counter_id': '2', 'int_key1': '1', 'str_key1': None, 'value': '1',
                                  'max_interval': '2024-05-01', 'prev_value': None}
        assert max_records[3] == {'counter_id': '2', 'int_key1': '2', 'str_key1': None, 'value': '10',
                                  'max_interval': '2024-05-10', 'prev_value': '9'}
        handle_event({"operation": "detect", "dataset": "simple_dataset", "interval": "day",
                      "from_day": "2024-05-11", "to_day": "2024-05-20", "override": True,
                      "method": "max"})
        max_records = list(get_query_results_dict(
            f"""SELECT counter_id, int_key1, str_key1, value, max_interval, prev_value
               FROM countdb.daily_max_counters
        ...
        assert max_records[1] == {'counter_id': '1', 'int_key1': None, 'str_key1': 'simple2', 'value': '2',
                                  'max_interval': '2024-05-01', 'prev_value': None}
        assert max_records[2] == {'counter_id': '2', 'int_key1': '1', 'str_key1': None, 'value': '100',
                                  'max_interval': '2024-05-15', 'prev_value': '1'}
        assert max_records[3] == {'counter_id': '2', 'int_key1': '2', 'str_key1': None, 'value': '20',
                                  'max_interval': '2024-05-20', 'prev_value': '19'}
        assert int(list(get_query_results_dict(
           f"""SELECT COUNT() AS cnt
               FROM countdb.daily_max_counters
               WHERE dataset = 'simple_dataset'
                     AND counter_id = 3"""))[0]["cnt"]) == 0
