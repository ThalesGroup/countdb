...
class TestLambdaIntegrationDataset:
    ...
    def invoke_lambda(event: dict) -> dict:
        ...
        response = lambda_client.invoke(
            FunctionName=os.environ["FUNCTION_NAME"], Payload=json.dumps(event), LogType="Tail")
        if response["ResponseMetadata"]["HTTPStatusCode"] != 200:
            raise Exception(f"Error response from lambda: {response}", response)
        else:
            logging.info(f"{'=' * 40}LOGS{'=' * 40}\n{b64decode(response['LogResult']).decode('UTF8')}")
            return json.loads(response['Payload'].read())

    def test_lambda_upload(self):
        result = self.invoke_lambda({"operation": "upload", "data": get_b64_resource("simple_dataset.json").decode()})
        assert "errors" not in result

    def test_lambda_collect(self):
        result = self.invoke_lambda({"operation": "collect", "dataset": "simple_dataset", "override": False})
