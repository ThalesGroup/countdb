...
class TestCountDBIntegration:
    ...
    def test_clear_dataset(self):
        ...
        result = handle_event(
            {"operation": "clear", "dataset": "dataset1", "table": "daily_counters"}
        )
        assert result == {"deleted": 0, "operation": "delete"}
    ...
    def test_compile_missing_fields(self):
        bad_json = {
            "dataset": "bad_dataset",
            "counters": [
                {"id": 1, "name": "bad", "sum": True, "sql": "SELECT 1 AS col"},
            ],
        }
        result = handle_event(
            {
        ...
        assert (
            result["errors"][0] == "Expected 3 columns: int_key, str_key, value. Got []"
        ), result["errors"][0]
...
class TestSimpleDataset:
    class SimpleDatasetHandler(AthenaMockQueryHandler):
        ...
        @staticmethod
        def query_match(
            m: re.Match, exec_params: Optional[list], context: MyAthenaBackendContext
        ):
            return QueryResults(
                [],
                [
                    {"Name": "int_key", "Type": "array"},
                    {"Name": "str_key", "Type": "array"},
    ...
    def test_simple_counter_single(self, simple_dataset):
        result = handle_event(
            {"operation": "collect", "dataset": "simple_dataset", "counter_id": 1}
        )
        assert {
            "exists": 0,
            "operation": "collect",
