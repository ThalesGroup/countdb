...
from datasets import get_datasets, _Counter, get_dataset, replace_sql_place_holders, create_dataset_temp_tables, \
    build_exists_query
from table_creator import get_table_creator
from utils import days_range, get_yesterday, get_session, _MAX_WORKERS


def collect(from_day: str = None, to_day: str = None, dataset_name: str = None,
            override: bool = False,
            counter_id: int = None) -> Dict:
    """
    ...
    existing_data = {} if override else get_existing_data("daily_counters", from_day, to_day, dataset_name,
                                                          counter_ids=[counter_id] if counter_id else None,
                                                          session=session)
    errors = 0
    success = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=_MAX_WORKERS,
                                               thread_name_prefix="collect_") as thread_pool:
        futures = []
        exists = []
        for dataset in [dataset_name] if dataset_name else get_datasets(session).keys():
            ...
            for day in days_range(from_day, to_day):
                ...
                if dataset in existing_data and day in existing_data[dataset]:
                    ...
                    futures.append(thread_pool.submit(_run_daily_collection_task, dataset, day, counter_id))
        if len(exists) > 0:
            logging.info(f"Data already exists for {len(exists)} days. {log_str}. Days: {exists}")
    ...
    result = {"operation": "collect", "status": "UP", "success": success,
              "exists": len(exists), "duration": str(elapsed_time)}
    if errors > 0:
...
def _run_daily_collection_task(dataset: str, day: str, counter_id: int) -> bool:
    ...
    try:
        if _calc_prerequisites(dataset, day, session):
            ...
            get_table_creator("daily_counters").create_day(dataset, day, counter_id, session)
        return True
...
def _calc_prerequisites(dataset_name: str, day: str, session: Session) -> bool:
    for q in get_dataset(dataset_name, session).prerequisites:
        ...
        if len(result["ResultSet"]["Rows"]) != 2 or result["ResultSet"]["Rows"][1]["Data"][0]["VarCharValue"] != "true":
...
class DailyCountersCreator(BasCountersCreator):
    ...
    @staticmethod
    def get_counter_sql(counter: _Counter, day: str, temp_tables: Dict[str, str]) -> str:
