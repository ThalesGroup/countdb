...
def folder_last_modified(prefix: str, session: Session = None) -> Dict[str, str]:
    ...
    contents = result.get("Contents")
    if contents:
        return {row["Key"][len(prefix):]: str(row["LastModified"].date()) for row in contents}
    else:
        return {}
...
def clear_s3_folder(s3_folder: str, session: Session = None) -> int:
    ...
    if len(res) == 0:
        ...
        raise Exception(f"Errors found: {len(res[0]['Errors'])}, First error: {res[0]['Errors'][0]}")
...
def add_s3_life_cycle_config(bucket: str,
                             prefix: str,
    ...
    for rule in _get_lifecycle_rules(s3, bucket):
        ...
        elif not _life_cycle_rule_changed(rule, delete_days, transition_days, transition_storage_class):
    ...
    if transition_days:
        rule["Transitions"] = [{"Days": transition_days, "StorageClass": transition_storage_class}]
    rules_to_add.append(rule)
    s3.put_bucket_lifecycle_configuration(Bucket=bucket, LifecycleConfiguration={"Rules": rules_to_add})
    logging.info(f"Added life cycle rule. Bucket: {bucket}, Prefix: {prefix}, "
...
def _life_cycle_rule_changed(rule: Dict,
                             delete_days: int,
                             transition_days: int,
    ...
    if (transition_days is not None) != ("Transitions" in rule):
        return True
    elif transition_days is not None and (len(rule["Transitions"]) != 1 or
                                          rule["Transitions"][0]["Days"] != transition_days or
                                          rule["Transitions"][0]["StorageClass"] != transition_storage_class):
        return True
    return False
