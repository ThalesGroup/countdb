...
from utils import get_session, date_add, get_last_day_of_month, weeks_range, month_range, get_last_finished_week, \
    get_yesterday, get_last_finished_month, _MAX_WORKERS, days_range


class PeriodCountersCreator(BasCountersCreator, ABC):
    @staticmethod
    def _get_counters_by_aggregation(counters: Iterable[_Counter]) -> Dict[str, List[int]]:
        result = {}
        for c in counters:
            if c.aggregate:
...
def aggregate(from_day: str, to_day: str, override: bool = False, dataset_name: str = None, interval_type: str = None,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=_MAX_WORKERS,
                                               thread_name_prefix="aggregate_") as thread_pool:
        for interval_type in [interval_type] if interval_type else ["week", "month"]:
            intervals = list(_get_aggregate_interval_range(interval_type, from_day, to_day))
            table_name = _get_table_name_by_interval(interval_type)
            futures = []
            if len(intervals) > 0:
                interval_existing_data = {} if override else \
                    _get_existing_data_for_intervals(table_name, interval_type, intervals, dataset_name,
                                                     counter_ids, session)
                daily_existing_data = _get_existing_data_for_intervals("daily_counters", interval_type, intervals,
                                                                       dataset_name, counter_ids=counter_ids,
                                                                       session=session)
                for dataset in [dataset_name] if dataset_name else get_datasets(session).keys():
                    for interval in intervals:
                        log_str = f"Aggregate. Dataset: {dataset}. Interval type: {interval_type}. " \
                                  f"Interval: {interval}. Counter ID: {str(counter_ids) if counter_ids else 'ALL'}"
                        if dataset in interval_existing_data and interval in interval_existing_data[dataset]:
                            logging.info(f"Data exists for for {log_str}")
                            exists += 1
                        elif not _daily_data_exists(daily_existing_data, dataset, interval_type, interval):
                            logging.warning(f"Missing  daily data for {log_str}")
                            missing_days += 1
                        else:
                            futures.append(thread_pool.submit(_run_agg_task, dataset, log_str,
                                                              table_name, interval, counter_ids))
        ...
        result = {"operation": "aggregate", "success": success, "missing_days": missing_days,
                  "exists": exists, "duration": str(elapsed_time)}
        if errors > 0:
            result["errors"] = errors
        return result


def _run_agg_task(dataset: str, log_str: str, table_name: str, interval: str, counter_ids: List[int]) -> bool:
    session = get_session()
    try:
...
def _get_existing_data_for_intervals(table_name: str, interval_type: str, intervals: List[str],
                                     dataset_name: str = None, counter_ids: List[int] = None,
    ...
    return get_existing_data(table_name, from_day, to_day, dataset_name, counter_ids=counter_ids, session=session)
...
def _get_aggregate_interval_range(interval: str, from_day: str, to_day: str):
    if interval == "week":
        ...
        if from_day is None and to_day is None:
            return month_range(f"{get_last_finished_month()}-01", get_last_day_of_month(get_last_finished_month()))
        else:
...
def _daily_data_exists(daily_data: Dict[str, Set[str]],
                       dataset: str,
                       interval_type: str,
                       interval: str) -> bool:
    if interval_type == "week":
